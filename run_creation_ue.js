    ...
    (record, runtime, search, xml, task, cm, moment, ns_utils, format) => {
        ...
        const beforeLoad = (scriptContext) => {
            ...
            if (scriptContext.type == 'view') {
                ...
                var isCreatedRun = scriptContext.newRecord.getValue({ fieldId: 'custbody_ddc_runs_created' });
                        ...
                        if (!countRelatedRuns(id)) {
                            ...
                            if (runs.length) {
                                ...
                                if (invokeMapReduce) {
                                    ...
                                    runs.map(m => cm.unParseDateValueFields(m))
                                    log.debug(`JobId:${id} => MapReduce runs length : ${runs.length}`, runs)
