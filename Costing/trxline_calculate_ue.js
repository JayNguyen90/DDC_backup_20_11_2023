        ...
        trigger.estimate = context => {
            ...
            try {
                ...
                if (currentUser.user_subsidiary == getScriptParameterSubsidiary || (currentUser.user_internalid == getScriptParameterUser && runtime.executionContext == 'WEBSERVICES')) {
                    ...
                    let costingMap = costFormulaIDs.length ? costingFormulaMap(costFormulaIDs) : {}
                    ...
                    for (let i = 0; i < lineCount; i++) {
                        ...
                        let unit = parseFloatOrZero(rec.getSublistValue({ sublistId: 'item', fieldId: 'custcol_ddc_unit_sale', line: i }))
                        ...
                        if (itemId in wsMap)
                            evaluateFormula(wsMap, itemId, 'coded_tx', rec, i, vars, 'custcol_ddc_estimated_kgs', 'custcol_ddc_est_kg_error_msg')
                        // vars.custcol_ddc_third_party_cost_calc = (vars.custcol_ddc_estimated_kgs ? vars.custcol_ddc_estimated_kgs : (qty / unit)) * item._3rd_pt_cost
                        if (runtime.executionContext == "SUITELET") {
                            log.debug("set from SCPQ", "set from SCPQ");
                            var estimtedKgs = parseFloatOrZero(rec.getSublistValue({ sublistId: 'item', fieldId: 'custcol_ddc_estimated_kgs', line: i }))
                            log.debug("set from SCPQ estimtedKgs ", estimtedKgs);
                            if (estimtedKgs != 0) {
                                vars.custcol_ddc_estimated_kgs = estimtedKgs
                            }
                        }
                        var thirdPartyCost = parseFloatOrZero(rec.getSublistValue({ sublistId: 'item', fieldId: 'custcol_ddc_third_party_cost', line: i }))
                        vars.custcol_ddc_third_party_cost_calc = (vars.custcol_ddc_estimated_kgs ? vars.custcol_ddc_estimated_kgs : (qty / unit)) * thirdPartyCost
            ...
            try {
                ...
                if (currentUser.user_subsidiary == getScriptParameterSubsidiary || (currentUser.user_internalid === getScriptParameterUser && runtime.executionContext === 'WEBSERVICES')) {
                    ...
                    for (let i = 0; i < lineCount; i++) {
                        ...
                        log.emergency({
                            title: 'LOG CURRENT VARS VALUE 1',
                            details: vars
                        })
                        ...
                        log.emergency({
                            title: 'LOG CURRENT VARS VALUE 2',
                            details: vars
                        })
                        ...
                        if (itemId in wsMap)
                            evaluateFormula(wsMap, itemId, 'coded_tx', rec, i, vars, 'custcol_ddc_estimated_kgs', 'custcol_ddc_est_kg_error_msg')
                        if (item_type == 'InvtPart' && parseFloatOrZero(vars.custcol_ddc_estimated_kgs) > 0) {
                            vars.custcol_ddc_actual_kgs = vars.custcol_ddc_estimated_kgs
                            vars.custcol_ddc_billable_qty_manual = vars.custcol_ddc_estimated_kgs
                            vars.quantity = vars.custcol_ddc_estimated_kgs
                        }
                        // vars.custcol_ddc_act_3party_cost_calc_tot = (vars.custcol_ddc_estimated_kgs ? vars.custcol_ddc_estimated_kgs : (qty / unit)) * item._3rd_pt_cost
                        var _3rd_pt_cost = parseFloatOrZero(rec.getSublistValue({ sublistId: 'item', fieldId: 'custcol_ddc_third_party_cost', line: i }))
                        log.debug("vars.custcol_ddc_estimated_kgs", vars.custcol_ddc_estimated_kgs);
                        vars.custcol_ddc_act_3party_cost_calc_tot = (vars.custcol_ddc_estimated_kgs && vars.custcol_ddc_estimated_kgs != 0) ? vars.custcol_ddc_estimated_kgs : (actualQuanity / unit) * _3rd_pt_cost
                        ...
                        log.emergency({
                            title: 'LOG CURRENT VARS VALUE 3',
                            details: vars
                        });
                        ...
                        log.emergency({
                            title: 'LOG CURRENT VARS VALUE 4',
                            details: vars
                        });
                        ...
                        log.emergency({
                            title: 'LOG CURRENT VARS VALUE 5',
                            details: vars
                        });
                        //
                        ...
                        //5
                        log.emergency({
                            title: 'LOG CURRENT VARS VALUE 6',
                            details: vars
                        });
            ...
            search.create({
                ...
                .run().each(each => {
                    map[each.id] = {
                        ...
                        reel_width: each.getValue({ name: 'custitem_ive_paper_width' }),//added as part of the paper weight calculator req
            ...
            if (fieldFormula) {
                ...
                for (key of fieldIds) {
                    ...
                    if (key == 'quantity') {
                        ...
                        if (recordType == 'salesorder' && fieldId == 'custcol_ddc_estimated_kgs') {
