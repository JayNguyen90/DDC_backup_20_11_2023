...
define(["N/record", 'N/search', 'N/task'], function (record, search, task) {
    var objScheduleScript = {
        SCRIPT_ID:'customscript_jcs_close_run_mr',
        DEPLOYMENT_PREFIX: 'close_run_',
        AUTOMATED_RECORD_ID: 'custscript_internalid_close_run'
    };
    function onRequest(context) {
        ...
        try {
            var runUpdateStatus = record.submitFields({
                type: "customrecord_ddc_run",
                id: recid,
                values: {
                    custrecord_ddc_run_status: 7,
    ...
    function createScriptDeployment(recordId) {
        ...
        var date=new Date()
        var deploymentId = '_close_run_' +recordId+"_"+(date.getTime()).toString().slice(-5);
        //var deploymentId = '_close_run_' +recordId+"_"+new Date().getTime();
        scriptDeployment.setValue({
            fieldId: 'scriptid',
            value: deploymentId
        });
